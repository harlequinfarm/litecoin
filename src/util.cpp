...
const char * const BITCOIN_CONF_FILENAME = "healthcoin.conf";
const char * const BITCOIN_PID_FILENAME = "healthcoind.pid";
...
#else
    const char* pszModule = "healthcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Healthcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Healthcoin";
#else
    // Unix
    return pathRet / ".healthcoin";
#endif
#endif
}
