...
Run with the -testnet option to run with "play healthcoins" on the test network, if you
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_healthcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_healthcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/healthcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_healthcoin.coverage/index.html`.
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9814.
...
- BitcoinMiner : Generates healthcoins (if wallet is enabled).
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:healthcoin-project/healthcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `healthcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `healthcoin-cli` and the GUI debug console use this table to determine how to
