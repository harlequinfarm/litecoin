Litecoin Core version 0.13.2 is now available from:
  ...
  <https://download.healthcoin.org/healthcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a healthcoin
...
This allows running healthcoind without having to do any manual configuration.
...
`healthcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /healthcoinrpc>
        ...
        # optional bypass healthcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< healthcoinrpc:<password>
    </Location>
...
Restarting healthcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in healthcoin.conf
...
healthcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/healthcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `healthcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `healthcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of healthcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of healthcoins being spent is exactly
...
- `healthcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `healthcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
